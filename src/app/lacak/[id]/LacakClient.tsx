...
import { ArrowLeft, MapPin, Calendar, Image as ImageIcon, Share2, ExternalLink, Send } from "lucide-react";
import { StatusBadge } from "@/components/StatusBadge";
import { LaporanRow } from "@/lib/supabase";

export default function LacakClient({ id }: { id: string }) {
  ...
  // Web Share API + WhatsApp Fallback (Gratis & Native)
  const handleShareWA = async () => {
    ...
    // Coba Web Share API bawaan HP (Bisa share text + link langsung ke WA)
    if (navigator.share) {
      try {
        await navigator.share({
          title: `Lapor Desa Rau: ${laporan.judul}`,
          text: textMessage,
          url: currentUrl,
        });
        return;
      } catch (err) {
        // User batalkan share, abaikan
      }
    }

    // Fallback: Direct URL WhatsApp
